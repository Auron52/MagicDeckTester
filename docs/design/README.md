...
| [shard-volley-hold.md](shard-volley-hold.md) (§measured dead end) | Requiring the sac-land burn to be the plan's ONLY face-damage source before it counts as the Spectacle enabler (strict), vs allowing any not-yet-active Spectacle spell (permissive). | **Byte-identical over 100k games.** The powerset never affords Light Up at full cost, so the only `{Shard Volley, Light Up}` plans come from the Plan-B spectacle builder, where the sac-land burn is the sole trigger by construction. Strict kept (it is the rule as reasoned); the permissive branch was deleted. |
| [value-leaf-regeneration-queue.md](value-leaf-regeneration-queue.md) (§8.3) | Shared `FSLineCache`/`TranspositionTable` across label candidates as the explanation for B&B producing EARLIER labels. | **Refuted by measurement** (`MTG_LABEL_COLD_CACHE=1`, `c8e65ba`): a fresh cache per candidate changes **0 of 256** labels. Not order-dependence, not budget. **Now closed** — the cause was `FSLineWin`'s first-win shortcut used outside `FullSearchLine`'s ladder, and the *unpruned* arm was the wrong one; see [label-horizon-ladder.md](label-horizon-ladder.md). Note the 0/256 probe ran on **burn**, the one deck where B&B labels never differed — a null measured where the effect wasn't. |
...
| [shard-volley-hold.md](shard-volley-hold.md) | `ADOPTED` | burn holds Shard Volley unless it wins this turn, enables Spectacle, or pumps a prowess attacker. Contains the attack-projection bug it exposed: hard-cast haste creatures were missing from `projected_atk` (Vial-only), which is harmless as a RANK and unsafe as a GATE. |
...
| [goblins-value-model-ab.md](goblins-value-model-ab.md) | `OPEN` | **A GT diff is not an A/B:** the goblins overnight GT predates six engine commits, so its FAILs bundled them with the value model. Isolated same-binary A/B (deck copy ± `value.json`) + the four ways this investigation measured the wrong thing (`classify_turn_later` only re-runs the new arm; `--budget-ms 0` vs a big number; the d5 case omits `--depth` so `value_play` owns it; `explain_game --old-bin` reads the current deck). |
