...
| [value-leaf-regeneration-queue.md](value-leaf-regeneration-queue.md) (§8.3) | Shared `FSLineCache`/`TranspositionTable` across label candidates as the explanation for B&B producing EARLIER labels. | **Refuted by measurement** (`MTG_LABEL_COLD_CACHE=1`, `c8e65ba`): a fresh cache per candidate changes **0 of 256** labels. Not order-dependence, not budget. The live suspect is `FSLineWin`'s first-win shortcut used outside `FullSearchLine`'s ladder. |
