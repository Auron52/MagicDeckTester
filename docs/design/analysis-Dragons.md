...
| claude_sweep (5d) | **RUN and CLEAR** — 30 Opus players at commit `99338658`, 0 unresolved flags. See "Claude-play sweep" below for the run and its adjudication. (This row previously read NOT RUN; the sweep was completed in a later session that appended the section below without clearing this row.) |
...
* **5d claude-play sweep — CLOSED (2026-08-26, commit `99338658`).** 30 Opus players, 30/30
  returned, win turns matched the search in all 30 games, 0 unresolved flags (adjudication table
  above). This bullet previously read "NOT RUN"; that text predated the sweep and was left stale
