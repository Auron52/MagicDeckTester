...
- [x] ETB effects — DONE, builds clean. New `FireOwnEtbTriggers()` + `PerformMuxusReveal()` in SpellEffects.h; called from EffectHandler::EnterBattlefield (executor, passes entry.tutor_target) + TurnSolver apply_one creature-enter (rollout, passes tutor_target). Handles etb_self_creates_tokens, etb_damage_any + etb_damage_each_opponent (face via life-loss; opp creatures pinged+pruned inline), Matron ETB tutor (PerformTutor), Muxus reveal-6-put-creatures-MV≤5 (via DrawN, rest to bottom, each put fires its own FireOwnEtbTriggers cascade). NOTE refinement: Matron search-branching over tutor target relies on existing tutor_to_hand CollectActions enumeration — verify composes in Stage 5.
