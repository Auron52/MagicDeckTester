...
# THE LIBATION CUT IS PROVISIONAL — validation coverage is not symmetric

Raised by the deck owner: *"we may be underselling Libation, since less manual testing was done
with it."* The concern checks out, and it is sharper than it was put.

## Zero hand-played validation for exactly the two cards being cut

Across the 24 hand-played reference games in `references/Mirrorwing_Dragon/`:

```
Ancestral Anger      17 games    12,724 mentions
Gold Rush            22 games    10,265
Fists of Flame       16 games     7,945
Goblin Instigator    19 games      3,881
Mirrorwing Dragon    21 games      3,528
Zada                 16 games      3,331
Luxurious Libation    0 games          0     <-- NEVER APPEARS
Impolite Entrance     0 games          0     <-- NEVER APPEARS
```

(The references were played on an older list — their hands hold **Kazandu Refuge**, which the
shipped deck does not run — so they predate both cards' inclusion.)

Committed scenario fixtures are similarly lopsided: Oracle's Restoration 3, Frontline Heroism 3,
Fortifying Draught 2, Ancestral Anger 1, **Luxurious Libation 1, Impolite Entrance 0, Gold Rush 0.**

So the screen compared a card whose play is validated by 17 hand-played games (Anger) against two
whose play has never been checked by a human at all. **That is a bias in the apparatus, not in the
cards**, and it points in exactly the direction the deck owner suspected.

## And there IS a concrete play defect, worth 7.3x the whole measured effect

Keyed on `chosenX` from the replayed traces, across the 110 unbiased games where Libation was cast:

```
X=0, cast T2-T3    n=23    +0.304   t = +1.95
X>0, cast T2-T3    n=11    +0.545   t = +2.76
X=0, cast T4+      n=33    -0.212   t = -1.78
X>0, cast T4+      n=43    -0.140   t = -0.98

early (T2-T3) vs late (T4+)        +0.553   se 0.157   t = +3.52
```

**Both X values lose early and win late, so this is a TIMING defect, not a bad X choice.** The
X=0 "generate more creatures" line is not itself wrong — it is right on turn 4 and later
(−0.212) and wrong on turns 2–3 (+0.304). Casting Libation *at all* before turn 4 is what costs.

There is no gate in `MirrorwingProvider` on *when* Libation may be cast. `CastOrderRank` places it
at 11 (ahead of the draws, Gold Rush, Fists and Draught) and the X-choice hook offers X=0 and
X=max-attacking-board-power, but nothing withholds the card on an early turn.

**What that is worth.** In the 500-pair unbiased sample:

```
ACTUAL arm delta (mix vs all-Anger)                       -0.0060
COUNTERFACTUAL: T2-T3 casts perform at the T4+ rate       -0.0436     7.3x
```

−0.0436 would move `or4_ie1_lib1_an1` from *tied* with all-Anger to clearly ahead of it, and into
contention with the leaders. **The Libation cut may be an artifact of a timing heuristic rather
than a property of the card.**

## Honest limits on this

* The T4 boundary was chosen after inspecting the per-turn buckets. The mechanism motivates it, but
  it is a hypothesis awaiting a pre-registered confirmation, not a settled result.
* The counterfactual is optimistic. It assumes the early-cast games would perform at the late-cast
  rate; they may be games that were already going badly (mana flood, nothing else castable), in
  which case some of that +0.38 is a *symptom* rather than a *cause*.
* Sub-bucket n is small (11–43).

## What this changes

**The recommendation to cut Luxurious Libation should be treated as PROVISIONAL.** The Entrance
cut is better supported (Entrance is measured as *rare*, not weak, and its own defect — unmodelled
trample — is disclosed), but Libation's cut now rests partly on a play defect we can name.

Two cheap things would settle it, in order:

1. **Gate the cast on turn/board and re-screen.** A provider rule that withholds Libation before
   turn 4 (or below a board-power threshold) is a few lines, and the existing screen harness
   A/Bs it directly. This is squarely a `heuristic-optimization` question.
2. **Hand-play reference games that actually contain Libation and Entrance.** Both cards currently
   have zero. Until they have some, any measurement involving them carries an unquantified
   play-quality risk that no amount of extra games reduces.

Neither is blocking for adopting the Heroism/Draught core — that is +0.45 and rests on cards with
good coverage. It is specifically the *last few slots* that should not be frozen yet.
