...
all along (`value-leaf.md` Rule 0: generate on ONE frozen commit) and is now unblocked — except
...
git rev-parse HEAD:src                      # record: this is the freeze key for the whole regen
```
...
pools all five phases, resumes incrementally, and stages without adopting. Before any resume
after a `git pull`, verify `git rev-parse HEAD:src` still matches the queue's `freeze.src` —
identical src content keeps a paused run resumable; anything else invalidates it.
...
Measurement/adoption happens against the suite on the dev box (or anywhere with the same
frozen commit): staged model A/B'd per the value-leaf skill, user approves adoption. Nothing
on the third machine adopts anything.
