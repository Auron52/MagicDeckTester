...
| 6 | Dwarven Hold burst amount auto-decided | storage-land, same mechanic as Mercadian Bazaar | sibling amount decision to #4 | **CLOSED (no decision needed)** — burst = payment shortfall (only sensible value); charge is automatic; charge-vs-burst is plan-controlled (see decision-indexed-choice-protocol.md) |
