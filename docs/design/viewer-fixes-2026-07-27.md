...
| 4 | firebreathing mana invisible; no player control | `ApplyFirebreathing` greedy, reads pool untapped (`SpellEffects.h:1706-1786`) | new "how much" decision (4 sites); faithful display | **DONE** (turn-keyed side-channel; engine+GUI, byte-identical) |
| 6 | Dwarven Hold burst amount auto-decided | storage-land, same mechanic as Mercadian Bazaar | sibling amount decision to #4 | **REOPENED — TODO (human decision).** Clairvoyant SEARCH needs no change (plan-driven, per user); but NON-clairvoyant HUMAN must control charge-vs-burst + amount. Wire like #4 (keyed side-channel), payment-embedded injection (see decision-indexed-choice-protocol.md) |
