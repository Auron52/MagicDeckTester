...
# Memory caps: the ENGINE derives them from the machine's RAM since 2026-09-15 (src/core/
# MemBudget.h -- TT / line-cache / plan-cache bounds plus an RSS watchdog), so this driver no
# longer exports any. The 2026-09-06 lesson stands (a raw mtg-analyze path with NO caps OOM'd a
# 23 GB box at 23.3 GB anon); it is now closed inside the binary rather than per launcher. To
# give a machine a different budget set MTG_MEM_BUDGET_MB / MTG_RSS_CAP_GB in the environment;
# an explicit per-pool env var (an A/B arm, a probe) always wins over the derived default.
