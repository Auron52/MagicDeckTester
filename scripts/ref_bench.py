...
ONE PROCESS. Every reference game of every deck runs as a single pooled `mtg --batch` (see
run_pooled). It used to be one `mtg` per game, which made the bench's cost scale with the deck's
SIDECARS rather than its games -- each process re-loaded Mirrorwing's 256 MB keep-model bincache,
~700 MB RSS apiece, and OOM-killed the box at --threads 24. Pooled, that load happens once.
...
# ---- the POOLED RUN -----------------------------------------------------------------------
#
# ONE `mtg --batch` for every reference game of every deck, not one `mtg` per game. Both reasons the
# repo's pooling rule gives apply here, and a third that is specific to this bench:
#
#   * MEMORY. Each process independently loads the deck's sidecars. Mirrorwing's exhaustive keep
#     model is a 256 MB bincache -> ~700 MB RSS per process; at --threads 24 that is ~17 GB, which
#     OOM-killed the box. Pooled, the whole 9-game run peaks at 719 MB -- the SAME 719 MB, because
#     one process loads it once. Memory now sits flat in the thread count instead of scaling with it.
#   * WALL CLOCK. That load was also ~3 s of every ~3.3 s game. Pooled, the 9 Mirrorwing references
#     run in 1.97 s total -- less than ONE game cost before.
#   * UTILISATION. `--batch` is the only route that reports `[batch] heartbeat: N/M workers busy`
#     and a SLOW-GAME repro. A loop of subprocesses has no utilisation signal at all.
#
# Requires the manifest's per-job "force_mulligan" (see BatchRunner Job::fm_count). Before that
# existed this bench structurally COULD NOT pool: forcing a recorded hand was CLI-only, and each
# reference game needs its own spec.
JOB_RE = re.compile(r"^(\S+): played=(\d+) avg=([\d.]+)")


def job_name(slug, g):
    """Unique per (deck, reference game) -- the key that maps a pooled result back to its row."""
    return "%s__%s" % (slug, g["name"][:-5] if g["name"].endswith(".json") else g["name"])


def run_pooled(entries, args, trace_dir):
    """Run EVERY collected game of EVERY deck as one pooled batch. -> {job_name: win_turn|None|"ERR"}.

    A job is one reference game: games=1, seed=<recorded>, game_index=<recorded gi>. That reproduces
    the old single-run `--seed S --game-index N` form exactly, because the batch runner computes
    global_gi = game_index + i and the shuffle seed = seed + i, with i=0 for a one-game job.

    `depth` is OMITTED unless asked, so each job falls to its deck's committed value_play (or the
    built-in d5) -- the policy that actually ships. ParseJob resolves that per job, so decks with
    different committed policies pool together without any of them being coerced to a shared one.
    """
    jobs = []
    for e in entries:
        for g in e["games"]:
            mull = g["mull"]
            j = {"name": job_name(e["deck"], g),
                 "deck": e["deck_obj"].deck_file,
                 "profile": e["deck_obj"].profile,
                 "games": 1, "seed": g["seed"], "game_index": g["gi"],
                 "max_turns": args.max_turns,
                 "force_mulligan": "%d:%s" % (mull.get("count", 0),
                                              ",".join(str(x) for x in (mull.get("bottom") or [])))}
            if args.depth is not None:
                # A reference deck may carry an ENABLED value_play lock; an explicit depth must
                # bypass it, exactly as --depth + --ignore-play-profile did on the CLI.
                j["depth"] = args.depth
    ...
    mpath = os.path.join(trace_dir or tempfile.gettempdir(), "ref_bench.manifest.json")
    ...
    if trace_dir:
        cmd += ["--game-trace-dir", trace_dir]
    print("pooled batch: %d reference games in ONE process (threads=%s)"
          % (len(jobs), args.threads or "auto"))
    sys.stdout.flush()
    p = subprocess.run(cmd, capture_output=True, text=True)
    if p.returncode != 0:
        sys.stderr.write(p.stdout[-3000:] + p.stderr[-3000:])
        sys.exit("ref_bench: pooled batch failed rc=%d" % p.returncode)
    ...
    for line in p.stdout.splitlines():
        ...
        # played=0 means the game was VOIDED (work ceiling) -- not a loss, and not a valid row.
        out[m.group(1)] = "ERR" if played == 0 else (
            None if v >= args.max_turns + 1 - 1e-9 else int(round(v)))
    missing = [j["name"] for j in jobs if j["name"] not in out]
    if missing:
        sys.stderr.write("PARSE-FAIL: %d job(s) produced no result line: %s\n"
                         % (len(missing), " ".join(missing[:8])))
    return out


def traced_hand(trace_dir, name, gi):
    """Opening-hand card numbers from the pooled run's trace (`<job name>_gi<N>.json`)."""
    path = os.path.join(trace_dir, "%s_gi%d.json" % (name, gi))
...
def collect_deck(slug, refdir, deck, args):
    """Read a deck's references into rows. Runs NOTHING -- every deck is collected first so the
    whole fleet goes into ONE pooled batch (see run_pooled)."""
    games = []
    ...
    for g in games:                       # a human win beyond the horizon is a loss on this scale
        if g["human"] is not None and g["human"] > args.max_turns:
            g["human"] = None
    return {"deck": slug, "games": games, "deck_obj": deck}


def report_deck(entry, wins, args, trace_dir):
    """Attach the pooled run's results to a deck's rows, flag them, and print the table."""
    slug, games, mt = entry["deck"], entry["games"], args.max_turns
    for g in games:
        name = job_name(slug, g)
        g["search"] = wins.get(name, "ERR")
        verdict = ""
        if trace_dir:
            want, got = g["want_hand"], traced_hand(trace_dir, name, g["gi"])
...
def main():
    ...
    ap.add_argument("--threads", type=int, default=0,
                    help="pooled batch workers (0 = auto). Memory is now FLAT in this number: "
                         "one process loads the deck sidecars once, so a high width no longer "
                         "multiplies a 700 MB footprint per worker")
    ...
    ap.add_argument("--no-verify-hands", action="store_true",
                    help="skip the forced-hand check (drops the pooled run's --game-trace-dir)")
    ap.add_argument("--log-root", default=None,
                    help="keep the pooled run's per-game traces here instead of a temp dir")
    ...
    tmp = None
    trace_dir = args.log_root
    if not args.no_verify_hands and not trace_dir:
        tmp = tempfile.mkdtemp(prefix="ref_bench_", dir="logs")
        trace_dir = tmp
    if args.no_verify_hands:
        trace_dir = None

    entries, skipped, fresh = [], [], []
    ...
    for slug, refdir in pairs:
        ...
        # today (see ref_dirs -- an archived list keeps its references in its own subfolder).
        ...
        if args.stale_only and cache_src and (cached.get(key) or {}).get("src") == cache_src:
            fresh.append(key)
            continue
        entries.append(collect_deck(key, refdir, decks[key], args))

    # EVERY deck's games in ONE batch -- collect first, run once, then report. Reporting per deck as
    # it completes would re-introduce the barrier the pooling exists to remove.
    wins = run_pooled(entries, args, trace_dir)
    results = [report_deck(e, wins, args, trace_dir) for e in entries]
    if fresh:
