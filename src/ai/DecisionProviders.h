...
// Gate for the learned leaf VALUE model (MTG_VALUE_MODEL). ADOPTED default-ON 2026-07-11
// (06e6ebe7): when a deck ships a value sidecar the learned estimate replaces the horizon rollout
// unless MTG_VALUE_MODEL=0. See UseValueModel() in the .cpp for the adoption record. Reads env once.
bool UseValueModel();
...
public:
    ...
    //
    // MEASURED AND REJECTED AS-IS, 2026-09-05 (the A/B this lever waited for -- it sat un-run
    // for a week; do not let a built lever idle unmeasured again). Pooled 6-arm sweep over
    // hinata's seven suite GT cells, 900 games/arm (logs/hinata_m2cap): RED ON ALL SEVEN CELLS
    // in both variants -- split+MTG_MAIN2_DROP 6.188 vs base 5.701 (+0.49/game), split under the
    // default-ON M2_RECONSIDER drop 6.029 (+0.33).
    //
    // WHY IT IS RED -- corrected 2026-09-06 by the unit profile (logs/hinata_cost). A first
    // write-up here blamed "interior-m2 spend amplification"; the measurement says the OPPOSITE:
    // the split is 42% CHEAPER (4.36M -> 2.52M units/100g; fs_pre collapses 4x, fs_main2
    // halves) and loses quality anyway. The mechanism is a CAPABILITY asymmetry, not spend:
    // fs_bp_wave goes 149,753 -> 0 -- FSLineTail (the m2 plan loop) HAS NO BREAKPOINT WAVE
...
public:
    ...
    // PLAN VALUE = MANA-EQUIVALENTS, NOT A COMBAT CLOCK (MTG_EDF_VAL_COMBO default ON since
    // 2026-09-15; MTG_EDF_VAL_RAMP default OFF, rejected -- measurements in the implementation's
    // ADOPTED block). This deck never wins by attacking, yet EvalCard prices its
