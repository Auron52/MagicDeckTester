...
void GameEngine::UntapStep(GameState& state)
{
    ...
    state.ritual_cast_this_turn = false;   // one-turn acceleration flag resets each turn (spasm gate; search-only reader, reset here for lockstep)
    state.crackle_since_ritual = false;  // Crackle-since-ritual flag resets each turn (spasm gate; search-only reader, reset here for lockstep)
