#!/usr/bin/env python3
"""Re-play every CHANGED Mirrorwing game at escalated budget AND depth. ONE pooled manifest.
...
merely a diluted one. That is the class the no-lossy-truncation bar rejects outright, and it is the
scorer for the Gold-Rush-positive decision -- NOT condemnation fire counts, which have predicted
the wrong sign three times in this arc.

Mirrorwing ships depth=5 budget=20ms, so the cells are 100x budget and 100x budget + 1 ply.

Takes N arms and censuses the UNION of games where they do not all agree, escalating EVERY arm at
every such game -- one pooled queue rather than a per-pair loop, so the box sees a single tail.

Usage: gen_gr_escalate_manifest.py <wins-dir> <arm> <arm> [<arm>...] > manifest.json
       (arm names must be keys of ARMS in gen_gold_rush_positive_manifest.py)
"""
import json
import pathlib
import sys
...
from gen_gold_rush_positive_manifest import ARMS, BLOCKS, MW   # noqa: E402
...
def main():
    ...
    for a in arms:
        if a not in ARMS:
            sys.exit(f"unknown arm {a!r}; known: {sorted(ARMS)}")

    jobs, changed = [], []
    for block, seed_base in BLOCKS.items():
        wins = {a: load(root / f"{a}.{block}.wins") for a in arms}
